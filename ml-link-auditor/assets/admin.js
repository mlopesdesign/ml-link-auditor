jQuery(function($){
  ...
  function simpleAction(action, $button, busyText, successMessage, extraData){
    const original = $button.text();
    state.scanBusy = true;
    updatePrimaryButtons({});
    abortCurrentXHR();
    $button.prop('disabled', true).addClass('is-busy').text(busyText || original);
    state.currentXHR = request(action, extraData || {});
  ...
  $(document).on('click', '.mlla-apply-suggestion', function(){
    const $btn = $(this);
    const payload = { result_id: $btn.data('result-id') };
    simpleAction('mlla_apply_suggestion', $btn, MLLA.strings.busy || 'Processando...', MLLA.strings.saved || 'Operação concluída.', payload);
  });
  ...
  // ===== v1.3.2: Licença + CSV Export =====

  function licenseAction(action, $btn, busyText, extraData) {
    if (!$btn || !$btn.length) return;
    const original = $btn.text();
    state.scanBusy = true;
    $btn.prop('disabled', true).addClass('is-busy').text(busyText || original);
    abortCurrentXHR();
    state.currentXHR = request(action, extraData || {});
    return state.currentXHR.done(function(resp) {
      if (resp.success) {
        toast(resp.data && resp.data.message ? resp.data.message : 'Operação concluída.', 'success', 4500);
        if (resp.data && resp.data.state_html) {
          $('#mlla-license-current-state').html(resp.data.state_html);
        }
        // Recarrega a página pra refletir novos gates Pro
        setTimeout(function(){ window.location.reload(); }, 800);
      } else {
        toast((resp.data && resp.data.message) || 'Erro na operação.', 'error', 6000);
        if (resp.data && resp.data.upgrade_url) {
          setTimeout(function(){ window.location.href = resp.data.upgrade_url; }, 1200);
        }
      }
    }).fail(function() {
      toast('Erro de comunicação com o servidor.', 'error', 6000);
    }).always(function() {
      state.currentXHR = null;
      state.scanBusy = false;
      $btn.prop('disabled', false).removeClass('is-busy').text(original);
    });
  }

  $('#mlla-activate-license').on('click', function() {
    const $btn = $(this);
    const key = ($('#mlla-license-key').val() || '').trim();
    if (!key) { toast('Informe a chave de licença.', 'warning', 4000); return; }
    licenseAction('mlla_activate_license', $btn, 'Ativando...', {
      license_key: key,
      email: $('#mlla-license-email').val() || ''
    });
  });

  $('#mlla-start-trial').on('click', function() {
    licenseAction('mlla_start_trial', $(this), 'Iniciando trial...', {
      email: $('#mlla-license-email').val() || ''
    });
  });

  $('#mlla-deactivate-license').on('click', function() {
    if (!window.confirm('Tem certeza que deseja desativar a licença neste site?')) return;
    licenseAction('mlla_deactivate_license', $(this), 'Desativando...', {});
  });

  $('#mlla-validate-license-now').on('click', function() {
    licenseAction('mlla_validate_license_now', $(this), 'Validando...', {});
  });

  $('#mlla-check-update').on('click', function() {
    const $btn = $(this);
    const original = $btn.text();
    $btn.prop('disabled', true).addClass('is-busy').text('Verificando...');
    request('mlla_check_update').done(function(resp) {
      if (resp.success) {
        toast(resp.data.message, 'success', 5000);
        $('#mlla-update-info').text(resp.data.message);
      } else {
        toast((resp.data && resp.data.message) || 'Falha ao verificar.', 'error', 6000);
      }
    }).fail(function() {
      toast('Erro de comunicação.', 'error', 6000);
    }).always(function() {
      $btn.prop('disabled', false).removeClass('is-busy').text(original);
    });
  });

  $('#mlla-export-csv').on('click', function() {
    const $btn = $(this);
    const hasCap = String($btn.data('has-cap')) === '1';
    if (!hasCap) {
      // Redireciona pra aba Licença (gate em PHP vai retornar 402 mesmo se tentar)
      window.location.href = MLLA.ajaxUrl.replace('admin-ajax.php', 'admin.php') + '?page=ml-link-auditor&tab=license';
      return;
    }
    const original = $btn.text();
    state.scanBusy = true;
    $btn.prop('disabled', true).addClass('is-busy').text('Preparando CSV...');
    // Download direto via form POST redirecionando para AJAX com download
    const form = $('<form>', { method: 'POST', action: MLLA.ajaxUrl });
    form.append($('<input>', { type: 'hidden', name: 'action', value: 'mlla_export_csv' }));
    form.append($('<input>', { type: 'hidden', name: 'nonce', value: MLLA.nonce }));
    $('body').append(form);
    form.submit();
    form.remove();
    setTimeout(function() {
